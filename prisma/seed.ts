...
// Size guides
//
// Inlined rather than kept in prisma/seed-data/: the folder held one file, and
// prisma/ is now migrations + schema + seed and nothing else.
// =============================================================================
interface SeedSizeGuide {
...
const SIZE_GUIDES: SeedSizeGuide[] = [
  {
    title: "Women — Kebaya",
...
const PRODUCTS = [
  ...
  },
  {
    ...
    brand: "LURE_BY_LINDWAY" as const,
    clothing: "DRESSES" as const,
    audiences: ["WOMEN" as const],
    price: 1_480_000,
    discount: 0,
...
async function main() {
  ...
  if (stockSynced.every((product) => product.stock === 0)) {
    ...
    console.warn("   The trigger ships as a migration — run: npm run db:migrate, then re-seed.");
  }
}
