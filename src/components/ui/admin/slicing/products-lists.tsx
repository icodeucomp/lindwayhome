...
import { audienceByKey, brandingByKey, garmentByKey } from "@/static/taxonomy";
...
const Taxonomy = ({ product }: { product: Product }) => {
  const parts = [brandingByKey(product.branding)?.label ?? product.branding, product.garment ? (garmentByKey(product.garment)?.label ?? product.garment) : null, ...product.audiences.map((key) => audienceByKey(key)?.label ?? key)];
